	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				// 你要清理数据
				val, err := rdb.GetDel(ctx, "phone_code:login:15234562222").Result()
				cancel()
				assert.NoError(t, err)
				// 你的验证码是 6 位
				assert.True(t, len(val) == 6)
			},
			reqBody:  `{"phone":"15234562222"}`,
			wantCode: 200,
			wantBody: web.Result{
				Msg: "发送成功",
			},
		},
		{
			...
			before: func(t *testing.T) {
				...
				_, err := rdb.Set(ctx, "phone_code:login:15234562222", "123456",
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				ctx, cancel := context.WithTimeout(context.Background(), time.Second*3)
				// 你要清理数据
				_, err := rdb.Set(ctx, "phone_code:login:15234562222", "123456", 0).Result()
				cancel()
				assert.NoError(t, err)
			},
			after: func(t *testing.T) {
